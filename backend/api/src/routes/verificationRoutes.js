...
router.get('/order/:orderId', orderVerificationLimiter, authenticate, validateParams(verifyOrderParamsSchema), async (req, res) => {
  try {
    ...
    // The order read is scoped to the caller (RLS: customers access own orders,
    // drivers view assigned orders), so run it through the authenticated
    // per-request client instead of the anon-key client.
