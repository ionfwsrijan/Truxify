...
router.get(
  ...
  async (req, res) => {
    ...
    let maxDetourKm;
    if (
      req.query.max_detour_km === undefined ||
      req.query.max_detour_km === ""
    ) {
      maxDetourKm = 50;
    } else {
      ...
      if (!Number.isFinite(parsed) || parsed <= 0) {
        return res
          .status(400)
          .json({ error: "max_detour_km must be a positive number" });
      }
    ...
    const latBucket = hasGps
      ? (Math.round(currentLat * 10) / 10).toFixed(1)
      : "x";
    const lngBucket = hasGps
      ? (Math.round(currentLng * 10) / 10).toFixed(1)
      : "x";
    ...
    try {
      ...
      if (hasGps && rawOffers && rawOffers.length > 0) {
        try {
          ...
          logger.error(
            "[orderRoutes] matchEnRouteLoads threw unexpectedly:",
            mlErr.message,
          );
          enriched = rawOffers;
          mlUnavailable = true;
      ...
      logger.error(
        "[orderRoutes] Failed to fetch en-route loads:",
        err.message,
      );
      res.status(500).json({ error: "Internal Server Error" });
    }
...
router.get(
  ...
  authenticate,
  userLimiter,
  requirePolicy("order:view-history"),
  async (req, res) => {
    try {
      const cursorParam = req.query.cursor ?? "1";
      const cursor =
        typeof cursorParam === "string" ? Number(cursorParam) : NaN;
      ...
      if (!Number.isInteger(cursor) || cursor < 1) {
        return res
          .status(400)
          .json({ error: "cursor must be an integer >= 1" });
      }
      ...
      if (cursor !== undefined) {
        ...
        if (!Number.isInteger(cursorNum) || cursorNum < 1) {
          return res
            .status(400)
            .json({ error: "cursor must be a positive integer" });
        }
      }

      const result = await orderLifecycleService.getOrderHistory(
        req.user.id,
        cursor,
        limit,
      );
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const timeline = await orderTimelineService.getOrderTimeline(
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const order = await orderValidationService.findOrderByIdOrDisplayId(
        orderId,
        "customer_id, driver_id, order_display_id",
      );
      orderValidationService.assertOrderFound(order);
      orderValidationService.assertOrderAccess(order, req.user);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const offer =
        await orderValidationService.assertLoadOfferAvailable(loadOfferId);
      ...
      await orderValidationService.assertNoDuplicateBid(
        loadOfferId,
        req.user.id,
      );

      const { data: bid, error: bidErr } = await orderRepository.createBid({
        load_id: loadOfferId,
      ...
      if (bidErr)
        return res
          .status(500)
          .json({ error: "Failed to record bid.", details: bidErr.message });
...
router.post(
  ...
  authenticate,
  userLimiter,
  requirePolicy("order:submit-rating"),
  validateParams(paramIdSchema),
  ...
  async (req, res) => {
    try {
      ...
      const order = await orderValidationService.findOrderByIdOrDisplayId(
        orderId,
        "id, order_display_id, customer_id, driver_id, status",
      );
      orderValidationService.assertOrderFound(order);
      orderValidationService.assertCustomerOwnership(order, req.user.id);
      orderValidationService.assertRatingDeliverable(order);
      await orderValidationService.assertNoDuplicateRating(
        order.order_display_id,
        req.user.id,
      );

      const { data: ratingData, error: rpcErr } =
        await orderRepository.executeRpc(
          "submit_rating_tx",
          {
      ...
      if (rpcErr) {
        return res
          .status(500)
          .json({ error: "Failed to submit rating.", details: rpcErr.message });
      }

      const { data: driverDetails } = await orderRepository.findDriverWallet(
        order.driver_id,
      );
      ...
      if (polygonAddress) {
        void awardReputationPoints(polygonAddress, stars).catch((repErr) => {
          logger.error(
            "[reputation] On-chain reputation update failed:",
            repErr.message,
          );
          orderRepository
            ...
            .catch((dbErr) =>
              logger.error(
                "[reputation] Failed to log failure:",
                dbErr.message,
              ),
            );
        });
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const order = await orderValidationService.findOrderByIdOrDisplayId(
        orderId,
        "order_display_id, customer_id",
      );
      if (!order)
        return res
          .status(403)
          .json({ error: "Access Denied: You do not own this order." });
      ...
      const { data: offer } =
        await orderRepository.findLoadOfferByOrderDisplayId(
          order.order_display_id,
        );
      if (!offer) return res.json([]);

      const { data: bids, error: bidErr } =
        await orderRepository.findBidsByLoad(offer.id, "pending", {
          orderBy: "bid_amount",
          ascending: true,
        });
      if (bidErr)
        return res
          .status(500)
          .json({ error: "Query failed.", details: bidErr.message });
      ...
      const [profilesRes, detailsRes] = await Promise.all([
        orderRepository.findProfilesByIds(
          driverIds,
      ...
      const trucksRes =
        truckIds.length > 0
          ? await orderRepository.findTrucksByIds(truckIds)
          : { data: [] };
      ...
      const enrichedBids = bids.map((bid) => {
        ...
        return {
          ...
          driver: {
            ...
            completion_rate: detail.completion_rate || 100.0,
          },
          truck,
        };
      });
...
router.post(
  ...
  authenticate,
  userLimiter,
  requirePolicy("order:accept-bid"),
  auditLog({ action: "order:accept-bid", resourceType: "order" }),
  requireIdempotency(86400),
  validateParams(acceptBidParamsSchema),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    ...
    if (!lockValue) {
      return res
        ...
        .json({
          error:
            "Another milestone update is in progress for this order. Please try again.",
        });
    }
...
router.post(
  ...
  auditLog({
    action: "delivery:verify",
    resourceType: "delivery_verification",
  }),
  ...
  async (req, res) => {
    try {
      const { escrowUpdateFailed } =
        await orderLifecycleService.verifyDeliveryFn(
          req.params.id,
          req.user.id,
          req.body.otp,
          req.token ? createUserClient(req.token) : undefined,
      ...
      if (escrowUpdateFailed) {
        return res.status(202).json({
          message:
            "Delivery verified successfully. Escrow payout requires reconciliation.",
  ...
  async (req, res) => {
    try {
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
        return res
          .status(400)
          .json({ error: "driver_lat and driver_lng must be valid numbers." });
      }
      ...
      if (
        ...
        if (!Number.isFinite(parsedRadius) || parsedRadius <= 0) {
          return res
            .status(400)
            .json({
              error: "geofence_radius_m must be a finite positive number.",
            });
        }
  ...
  async (req, res) => {
    try {
      ...
      const orderForAmount =
        await orderValidationService.findOrderByIdOrDisplayId(
          req.params.id,
      ...
      if (escrowUpdateFailed) {
        logger.warn(
          `[confirm-otp] escrowUpdateFailed for order ${req.params.id} — reconciliation required`,
        );
        return res.status(202).json({
          message:
            "Delivery confirmed. Escrow payout is pending reconciliation — your payment will be credited shortly.",
...
router.post(
  ...
  userLimiter,
  resendOtpLimiter,
  requirePolicy("delivery:resend-otp"),
  validateParams(paramIdSchema),
  async (req, res) => {
    try {
      const orderId = req.params.id;
      const order = await orderValidationService.findOrderByIdOrDisplayId(
        orderId,
        "id, order_display_id, driver_id, customer_id, status",
      );
      orderValidationService.assertOrderFound(order);
      orderValidationService.assertDriverAssignment(order, req.user.id);

      const { expiresInMinutes } =
        await deliveryVerificationService.resendDeliveryOtp({
          orderId,
...
router.put(
  ...
  userLimiter,
  changeDropLimiter,
  requirePolicy("order:change-drop"),
  auditLog({ action: "order:change-drop", resourceType: "order" }),
  validateParams(paramIdSchema),
  validateBody(changeDropSchema),
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        logger.error(
          "Pricing computation error for change-drop:",
          pricingErr.message,
        );
        return res
          .status(400)
          .json({
            error: "Unable to compute new pricing for the requested drop.",
      ...
      const { data: updatedOrder, error: updateErr } =
        await orderRepository.executeRpc(
          "update_order_and_load_offer",
          {
            p_order_id: order.id,
            p_order_display_id: order.order_display_id,
      ...
      if (updateErr) {
        logger.error(
          "Order and load offer atomic update failed for change-drop:",
          updateErr.message,
        );
        return res
          .status(500)
          .json({
            error: "Failed to update order atomically.",
...
router.post(
  ...
  authenticate,
  userLimiter,
  requirePolicy("order:cancel"),
  auditLog({ action: "order:cancel", resourceType: "order" }),
  requireIdempotency(86400),
  validateParams(paramIdSchema),
  ...
  async (req, res) => {
    ...
    if (!lockValue) {
      return res
        ...
        .json({
          error:
            "Another deposit confirmation is in progress for this order. Please try again.",
        });
    }
    ...
    try {
      const order = await orderValidationService.findOrderByIdOrDisplayId(
        orderId,
        "id, status, order_display_id, customer_id, escrow_booking_id, escrow_status, escrow_amount_wei, escrow_driver_wallet, pending_bid_acceptance",
      );
      orderValidationService.assertOrderFound(order);
      orderValidationService.assertCustomerOwnership(order, req.user.id);
      orderValidationService.assertEscrowState(
        order,
        ["funding"],
        "Order is not in funding state",
      );
      if (order.status === "cancelled")
        return res
          .status(409)
          .json({
            error: "Order is already cancelled. Cannot confirm deposit.",
          });

      const { data: customerProfile } =
        await orderRepository.findCustomerWallet(req.user.id);
      const customerWallet = customerProfile?.polygon_wallet_address ?? null;
      const bookingId =
        order.escrow_booking_id ||
        (order.order_display_id
          ? getEscrowBookingId(order.order_display_id)
          : orderId);
      ...
      const finalizeAcceptance = async () => {
        ...
        if (!pending) return;
        const { error: acceptErr } = await orderRepository.executeRpc(
          "accept_bid_tx",
          {
        ...
        if (acceptErr) {
          logger.error(
            "[confirm-deposit] accept_bid_tx failed:",
            acceptErr.message,
          );
          ...
          try {
            ...
            logger.error(
              "[confirm-deposit] Escrow refund also failed:",
              refundErr.message,
            );
          ...
          if (!refundConfirmed) {
            ...
            const refundError =
              refundResult?.error || "escrow refund was not submitted";
            await orderRepository
              ...
              .catch((stateErr) => {
                logger.error(
                  "[confirm-deposit] Failed to mark escrow refund pending:",
                  stateErr.message,
                );
              });
            throw new DomainError(503, {
              error:
                "Deposit confirmed but the driver assignment could not be finalized. The escrow refund is pending and will be completed automatically. Please try again shortly.",
          ...
          await orderRepository
            .revertEscrowStatus(orderId)
            .catch((revertErr) => {
              logger.error(
                "[confirm-deposit] Failed to revert escrow status:",
                revertErr.message,
              );
            });
          throw new DomainError(409, {
            error:
              "Deposit confirmed but the driver assignment could not be finalized. The escrow deposit has been refunded. Please try again.",
        ...
        sendPushNotification(
          ...
          logger.error(
            `[FCM] Failed to notify driver of bid acceptance: ${err.message}`,
          ),
        );
      };
      ...
      if (resolvedAmount.error) {
        return res
          .status(422)
          .json({ error: resolvedAmount.error, code: resolvedAmount.code });
      }
      ...
      if (result.alreadyFunded) {
        const { data: updatedData, error: updateErr } =
          await orderRepository.updateOrderWithFilter(
            orderId,
            {
        ...
        if (!updateErr && updatedData) {
          ...
          return res.json({
            message: "Escrow deposit confirmed (recovered).",
        ...
        return res
          ...
          .json({
            message:
              "Escrow deposit confirmed on-chain. Database sync pending.",
            txHash: result.txHash,
          });
      }
      ...
      if (updateErr) {
        ...
        return res
          ...
          .json({
            error:
              "Database update failed after deposit confirmation. Please contact support.",
          });
      }

      if (!updatedData) {
        logger.error(
          '[confirm-deposit] No row updated — escrow_status may not have been "funding"',
        );
        return res
          .status(409)
          .json({
            error:
              "Order was not in funding state. Please refresh and try again.",
          });
      }
...
router.post(
  ...
  authenticate,
  userLimiter,
  requirePolicy("order:predict-demand"),
  predictDemandLimiter,
  validateBody(predictDemandSchema),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!order.driver_id) {
        return res
          .status(404)
          .json({ error: "No driver assigned to this order." });
      }

      if (!mongoDb) {
        return res
          .status(503)
          .json({ error: "Telemetry database not available." });
      }
      ...
      if (!latestTelemetry || latestTelemetry.length === 0) {
        return res
          .status(404)
          .json({ error: "No live telemetry found for this driver." });
      }
...
router.get(
  ...
  userLimiter,
  telemetryLimiter,
  requirePolicy("order:view-route", async (req) => {
    const { data: order } =
      await orderValidationService.findOrderByIdOrDisplayId(
        req.params.id,
        "id, customer_id, driver_id",
  ...
  async (req, res) => {
    ...
    try {
      const order = await orderValidationService.findOrderByIdOrDisplayId(
        orderId,
        "id, customer_id, driver_id, status, pickup_lat, pickup_lng, drop_lat, drop_lng",
      );
      orderValidationService.assertOrderFound(order);
      ...
      if (order.drop_lat == null || order.drop_lng == null) {
        return res
          .status(500)
          .json({ error: "Order is missing destination coordinates." });
      }
      ...
      if (!order.driver_id) {
        ...
        if (
          ...
          return res
            .status(500)
            .json({ error: "Order has invalid coordinates." });
        }

        const feature = buildStraightLineGeometry({
          originLat,
          originLng,
          destLat,
          destLng,
      ...
      if (!Number.isFinite(originLat) || !Number.isFinite(originLng)) {
        return res
          .status(404)
          .json({
            error: "Latest telemetry record is missing valid coordinates.",
          });
      }
      ...
      if (!Number.isFinite(destLat) || !Number.isFinite(destLng)) {
        logger.error(
          `[route] Order ${order.id} has non-numeric destination coordinates.`,
        );
        return res
          .status(500)
          .json({ error: "Order has invalid destination coordinates." });
      }

      let feature = await getRouteGeometry({
        originLat,
        originLng,
        destLat,
        destLng,
      ...
      if (!feature) {
        logger.warn(
          `[route] OSRM unavailable for order ${order.id}, falling back to straight line.`,
        );
        feature = buildStraightLineGeometry({
          originLat,
          originLng,
          destLat,
          destLng,
    ...
    try {
      ...
      if (files.signature && files.signature[0]) {
        ...
        try {
          ...
          return res
            ...
            .json({
              error: `Invalid signature file: ${validationErr.message}`,
            });
        }
        ...
        if (upErr) {
          ...
          return res
            .status(500)
            .json({ error: "Failed to upload signature to storage" });
        }
      ...
      if (files.photo && files.photo[0]) {
        ...
        try {
          ...
          return res
            .status(validationErr.status || 400)
            .json({ error: `Invalid photo file: ${validationErr.message}` });
        }
        const ext = file.mimetype === "image/png" ? "png" : "jpg";
        ...
        if (upErr) {
          ...
          return res
            .status(500)
            .json({ error: "Failed to upload photo to storage" });
        }
      ...
      if (!uploadedAny) {
        return res
          ...
          .json({
            error:
              "At least one valid proof file (signature or photo) is required",
          });
      }
      ...
      if (signatureUrl !== order.pod_signature_url)
      ...
      if (updateErr) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to update order with PoD data" });
      }
